...
export function Shell({ children }: { children: ReactNode }) {
  ...
  const NAV = [
    { to: "/", label: t("navHome") },
    { to: "/assess", label: t("navCheck") },
    { to: "/library", label: t("navConditions") },
    { to: "/history", label: t("navResults") },
  ] as const;
  ...
  return (
    <div className="min-h-dvh flex flex-col">
      ...
      <header className="border-b border-border bg-paper sticky top-0 z-40">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-2 px-3 py-3 sm:px-6">
          <Link to="/" className="flex items-center gap-2 min-w-0 text-primary">
            ...
            <span className="min-w-0 text-foreground">
              ...
              <span className="mt-1 hidden sm:block text-[0.7rem] text-muted-foreground">
                {t("tagline")}
              </span>
            </span>
          </Link>
              ...
              {NAV.map((item) => {
                ...
                return (
                  <Link
                    ...
                    to={item.to}
                    className={cn(
                      "rounded-md px-2 py-2 text-xs font-semibold transition-colors duration-150 min-h-11 inline-flex items-center sm:px-3 sm:text-sm whitespace-nowrap",
                      active
      ...
      <footer className="border-t border-border bg-paper">
        <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
          <p className="text-sm leading-relaxed text-muted-foreground max-w-3xl">
            <strong className="text-foreground">PREVIA</strong>
            {" — "}
            {t("footer")}
          </p>
